...
export class Exec {
    ...
    update(){
        ...
        for (let i = GameState.enemies.length - 1; i >= 0; i--) {
            ...
            if (!e.isAlive()) {
                ...
                this.activate_visualizer();
                continue;
            }
            if (!e.generating && MyMath.isHittingCharacter(e.pos,e.size,GameState.player.pos,GameState.player.size)){
                // 敵とプレイヤーの衝突
        ...
        for (let i = GameState.items.length - 1; i >= 0; i--) {
            ...
            if (!item.isAlive()) {
                ...
                GameState.items[i] = new I1(this.scene);
                GameState.items[i].init2(i);
                GameState.items[i].set_type(GLOBALS.ITEM.TYPE.NONE);
                GameState.items[i].set_visible(true);
                continue;
            }
        }
    ...
    activate_visualizer(){
        if (GameState.visualizer >= 1 && GameState.enemies.length <= 1){
            this.set_item_visible(GLOBALS.ITEM.TYPE.EXIT);
        }
        if (GameState.visualizer >= 2 && GameState.enemies.length === 0){
            this.set_item_visible(GLOBALS.ITEM.TYPE.KEY);
        }
        if (GameState.visualizer >= 3 && GameState.enemies.length <= 3){
            this.set_item_visible(GLOBALS.ITEM.TYPE.NONE);
        }
        if (GameState.visualizer >= 4 && GameState.enemies.length <= 2){
