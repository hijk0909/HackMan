...
const itemBoxCheckers = {
  ...
  4: { check: () => { if (GameState.enemies.length <= 1) {
          open_item_box(GLOBALS.ITEM.TYPE.VISUALIZER);} },
        hint: "ENEMIES TO 1",
        item: "VISUALIZER"
  },
  ...
  // FLOOR 10 : 敵を全滅させる → グローブ
  10: { check: () => { if (GameState.enemies.length === 0) {
          open_item_box(GLOBALS.ITEM.TYPE.GLOVE);} },
        hint: "ENEMIES TO 0",
        item: "GLOVE"
  },
  ...
  // FLOOR 13 : 敵を全滅させる → バリア
  13: { check : () => { if (GameState.enemies.length === 0) {
          open_item_box(GLOBALS.ITEM.TYPE.BARRIER);} },
        hint: "ENEMIES TO 0",
        item: "BARRIER"
      },
  // FLOOR 14 : 下辺の壁に接触する → スコープUP
  ...
  // FLOOR 15 : 敵を残り１以下にする → グローブUP
  15: { check : () => { if (GameState.enemies.length <= 1) {
          open_item_box(GLOBALS.ITEM.TYPE.GLOVE);} },
        hint: "ENEMIES TO 1",
        item: "GLOVE"
      },
  // FLOOR 16 : 敵を残り１以下にする → フリップUP
  16: { check : () => { if (GameState.enemies.length === 0) {
          open_item_box(GLOBALS.ITEM.TYPE.FLIP);} },
        hint: "ENEMIES TO 0",
        item: "QUCIK FLIP"
      },
  // FLOOR 17 : 上辺のパネルに移動する → USB_UP
  17: { check : () => { if (count_item(GLOBALS.ITEM.TYPE.PICT_CHET) >= 1){
          open_item_box(GLOBALS.ITEM.TYPE.USB);
          item_all_blink_out(GLOBALS.ITEM.TYPE.PICT_CHET);
        }},
        hint: "CHANGE PICT TO CHET",
        item: "USB"
      },
  // FLOOR 18 : 上辺のパネルに移動する → スピードUP
  18: { check : () => { const { loc_x, loc_y, rel_pos_x, rel_pos_y} = MyMath.get_loc_from_pos(GameState.player.pos.x, GameState.player.pos.y);
             if (loc_y === 0){
                open_item_box(GLOBALS.ITEM.TYPE.SPEED); }},
  ...
  // FLOOR 21 : 敵を全滅させる → グローブUP
  21: { check : () => { if (GameState.enemies.length === 0) {
          open_item_box(GLOBALS.ITEM.TYPE.GLOVE);} },
        hint: "ENEMIES TO 0",
        item: "GLOVE"
      },
  // FLOOR 22 : 敵を3以下にする → USB
  22: { check : () => { if (GameState.enemies.length <= 3) {
          open_item_box(GLOBALS.ITEM.TYPE.USB);} },
        hint: "ENEMIES TO 3",
        item: "USB"
      },
  ...
  // FLOOR 60 : ピクトを４枚、ALPHAに揃える → キー
  60: { check : () => { if (count_item(GLOBALS.ITEM.TYPE.PICT_ALEF) === 4) {
          open_item_box(GLOBALS.ITEM.TYPE.KEY);} },
        hint: "ALL PICT TO ALEF",
        item: "KEY"
      },
  };
