...
export class UIScene extends Phaser.Scene {
    ...
    // ◆グローブ
    collection_update_glove(blink){
        if (GameState.glove === 0){
            this.collections[COLLECTION_POS.GLOVE].setVisible(false);
        } else {
            const type = GLOBALS.ITEM.TYPE.ST_GLOVE + GameState.glove - 1;
            this.collections[COLLECTION_POS.GLOVE].setFrame(type).setVisible(true);
            if (blink){
                this.collection_blink(COLLECTION_POS.GLOVE,type);
            }
        }
    }
    ...
    // ◆ビジュアライザ
    collection_update_visualizer(blink){
        if (GameState.visualizer === 0){
            this.collections[COLLECTION_POS.VISUALIZER].setVisible(false);
        } else {
            const type = GLOBALS.ITEM.TYPE.ST_VISUALIZER + GameState.visualizer - 1;
            this.collections[COLLECTION_POS.VISUALIZER].setFrame(type).setVisible(true);
            if (blink){
                this.collection_blink(COLLECTION_POS.VISUALIZER,type);
            }
        }
    }
}