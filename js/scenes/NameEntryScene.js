...
export class NameEntryScene extends Phaser.Scene {
    ...
    create(){
        ...
        // フロア表示
        this.floor = (GameState.floor > GLOBALS.FLOOR_MAX) ? "ALL" : GameState.floor;

        //ランキング画面
        this.ranking = new Ranking(this);
        this.rank = Ranking.get_new_rank(GameState.ranking.session, GameState.score);
        GameState.ranking.session = Ranking.insert_temp_score(GameState.ranking.session, GameState.score, this.floor);
        this.ranking.show_ranking_table(GameState.ranking.session);
        //ネームエントリ画面（キーボード）
        this.name_entry = new name_entry(this, (name, score, floor) => {this.set_ranking(name, score, floor);});
    ...
    set_ranking(name){
        ...
        this.ranking.show_ranking_table(GameState.ranking.session);
        this.ranking.set_net_ranking(name, GameState.score, this.floor);
        this.time.addEvent({
            delay: 3000,
            callback: () => {
