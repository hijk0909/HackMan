...
export class Ranking {
    ...
    show_ranking_table(ranking) {
        ...
        const headerText = 'RANK NAME      FLOOR   SCORE';
        ...
        ranking.forEach((item, index) => {
            ...
            const floor = (item.floor).toString().padStart(3, ' ');
            // 表示する文字列をフォーマット
            const displayText = `${rank}. ${name}  [${floor}] ${score}`;
    ...
    static insert_temp_score(ranking, newScore, newFloor) {
        const newRank = this.get_new_rank(ranking, newScore);
        if (newRank === -1) return ranking; // 変化なし
        const newEntry = { name: "", score: newScore, floor: newFloor };
        // 配列をコピーして挿入
        const newRanking = [
